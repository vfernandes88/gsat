...
where C.CHAPASUBST = '{0}' AND C.CODCOLIGADA = {1}  ORDER BY DESCRICAO ASC", matricula, coligada);
        ...
        public static DataTable GetAllCentrosCustoVazios()
        {
            if (ambiente_producao)
            {
                #region PRODUCAO
                try
                {
                    command = new SqlCommand("SELECT distinct CODIGO, DESCRICAO, (CODIGO + ' - ' + DESCRICAO) AS COD_DESC, (CODIGO + '_' + ESTADO) AS CODSECAO_ESTADO, (CODIGO + '_' + ESTADO + '_' + cast(CODCOLIGADA as varchar)) AS CODSECAO_ESTADO_COLIGADA FROM LK_RM..INT_SHAREP_RM.VW_CCUSTO_ATIVO ORDER BY DESCRICAO ASC", BaseDAL.GetConnection());
                    adapter = new SqlDataAdapter(command);
                    tableDados = new DataTable();
                    adapter.Fill(tableDados);
