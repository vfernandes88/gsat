...
(SELECT CODNIVEL FROM INT_SHAREP_RM.VW_HISTORICO_SALARIAL_REM WHERE CHAPA=C.CHAPA AND CODCOLIGADA=C.CODCOLIGADA AND EXTRACT(YEAR FROM DTMUDANCA)<=R.ANOCOMP AND FUNCAO IS NOT NULL AND CODNIVEL IS NOT NULL AND CODFAIXA IS NOT NULL AND ROWNUM = 1) CODNIVELSAL,                                 
(SELECT CODFAIXA FROM INT_SHAREP_RM.VW_HISTORICO_SALARIAL_REM WHERE CHAPA=C.CHAPA AND CODCOLIGADA=C.CODCOLIGADA AND EXTRACT(YEAR FROM DTMUDANCA)<=R.ANOCOMP AND FUNCAO IS NOT NULL AND CODNIVEL IS NOT NULL AND CODFAIXA IS NOT NULL AND ROWNUM = 1) GRUPOSALARIAL,
        ...
        public static DataTable GetFolhaPagamento(string centroCusto)
        {
            try
            {
                if (ambiente_producao)
                {
                    #region Produção
                    string commandText = string.Format(@"SELECT * FROM OPENQUERY(LK_RM,  
                                'SELECT DISTINCT C.CHAPA, C.NOME,  
(SELECT DISTINCT FUNCAO   FROM INT_SHAREP_RM.VW_HISTORICO_SALARIAL_REM WHERE CHAPA=C.CHAPA AND CODCOLIGADA=HS.CODCOLIGADA AND EXTRACT(YEAR FROM DTMUDANCA)<=R.ANOCOMP AND FUNCAO IS NOT NULL AND ROWNUM = 1) CARGO,
(SELECT DISTINCT CODNIVEL FROM INT_SHAREP_RM.VW_HISTORICO_SALARIAL_REM WHERE CHAPA=C.CHAPA AND CODCOLIGADA=HS.CODCOLIGADA AND EXTRACT(YEAR FROM DTMUDANCA)<=R.ANOCOMP AND CODNIVEL IS NOT NULL AND ROWNUM = 1) CODNIVELSAL,                                 
(SELECT DISTINCT CODFAIXA FROM INT_SHAREP_RM.VW_HISTORICO_SALARIAL_REM WHERE CHAPA=C.CHAPA AND CODCOLIGADA=HS.CODCOLIGADA AND EXTRACT(YEAR FROM DTMUDANCA)<=R.ANOCOMP AND CODFAIXA IS NOT NULL AND ROWNUM = 1) GRUPOSALARIAL,
(SELECT DISTINCT SALARIO  FROM INT_SHAREP_RM.VW_HISTORICO_SALARIAL_REM WHERE CHAPA=C.CHAPA AND CODCOLIGADA=HS.CODCOLIGADA AND EXTRACT(YEAR FROM DTMUDANCA)<=R.ANOCOMP AND SALARIO IS NOT NULL AND ROWNUM = 1) SALARIO, 
                                    P.DTBASE Admissao  
                                    FROM INT_SHAREP_RM.VW_CCUSTO_INTRANET C  
                                    JOIN INT_SHAREP_RM.VW_PERFIL_INTRANET P  
                                    ON P.CHAPA = C.CHAPA AND P.CODCOLIGADA = C.CODCOLIGADA  
...
FROM INT_SHAREP_RM.VW_DEPENDENTES_FUNC
    ...
    ORDER BY TIPO DESC' )", matricula, coligada, ano, mes), BaseDados.GetConnection());
        ...
        public static DataTable GetDadosFuncionario(int codigoColigada, string matricula)
        {
            ...
            using (SqlConnection sqlConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["ConnectionStringProducao"].ConnectionString))
            {
